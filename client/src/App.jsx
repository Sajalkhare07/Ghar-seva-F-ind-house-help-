...
// client/src/App.jsx
// ── Paste into: client/src/App.jsx ────────────────────────────────────────────
// HomePage now manages its own data — no props needed from App
...
const App = () => {
  ...
  const renderPage = () => {
    switch (page) {
      case "home":
        // HomePage is now self-contained — only needs setPage for navigation
  ...
  return (
    <>
      ...
      {/* Helper profile modal — available from Browse page */}
      {selectedHelper && (
        <ProfileModal
          helper={selectedHelper}
          onClose={() => setSelectedHelper(null)}
          onSave={handleSave}
          saved={savedIds.includes(selectedHelper.id || selectedHelper._id)}
        />
      )}
